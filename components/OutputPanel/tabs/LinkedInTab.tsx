...
export default function LinkedInTab({ linkedinPost, shareUrl, showToast }: LinkedInTabProps) {
  ...
  const handleShare = (platform: string) => {
    ...
    switch (platform) {
      case 'linkedin':
        ...
        break;
      case 'twitter':
        const twitterText = `${safeString(linkedinPost)} - Analyze your code with AI and share it with the world! #Zbloue #CodeReview #AI #Developer`;
        window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(twitterText)}&url=${encodeURIComponent(shareUrl)}`, '_blank');
        break;
      case 'whatsapp':
        const whatsappText = `${safeString(linkedinPost)} - Analyze your code with AI and share it with the world! #Zbloue #CodeReview #AI #Developer`;
        window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(whatsappText + ' ' + shareUrl)}`, '_blank');
        break;
      case 'telegram':
        window.open(`https://t.me/share/url?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(safeString(linkedinPost))}`, '_blank');
        break;
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-[#1a1a2e]">LinkedIn Post</h2>
        </div>
      ...
      <div className="bg-[#fafbfc] p-4 rounded-md border-2 border-[#d0d0d8] whitespace-pre-wrap leading-relaxed text-sm md:text-base">
        {safeString(linkedinPost)}
      </div>
    </div>
  );
}